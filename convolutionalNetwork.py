...
class ConvolutionalNetwork(NeuronalNetwork):
    ...
    def layers_construction(self, training_sample = False):
        ...
        if not training_sample:
            self.model.add(Conv2D(32, (3,3), input_shape=self.data.images_training[0].shape))
        else:
            self.model.add(Conv2D(32, (3,3), input_shape=self.data.images_training_sample[0].shape))
